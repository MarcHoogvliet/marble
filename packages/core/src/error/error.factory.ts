...
export const stringifyStackTrace = (stackTrace: NodeJS.CallSite[]) =>
  stackTrace.map((s) => chalk.gray(`    @ ${s}`)).join('\n  ');

export const coreErrorStackTraceFactory = (opts: CoreErrorOptions) => (
  message: string,
  ...
  const methodName =
    opts.contextMethod + ' : ' + (method.getMethodName() ?? '-');
...
export const coreErrorFactory = (message: string, opts: CoreErrorOptions) =>
  new CoreError(message || 'Something is not right...', {
