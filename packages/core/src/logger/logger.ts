...
export const logger = createReader<Logger>(() => (opts) => {
  ...
  const tag: string =
    chalk.gray(truncItem(opts.tag)) + ' ' + colorize(`[${opts.type}]`);
  const message: string =
    opts.level === LoggerLevel.ERROR ? chalk.red(opts.message) : opts.message;

  return print(sign + sep + pid + sep + now + sep + tag + sep + message);
});

export const mockLogger = createReader<Logger>(() => (_) =>
  io.of(constUndefined)
);
