...
describe('WebSocket server', () => {
  describe('JSON transformer', () => {
    test('echoes back', async (done) => {
      ...
      const webSocketServer = await bootstrapWebSocketServer(
        { server: httpServer },
        listener
      );
    ...
    test('echoes back to all clients', async (done) => {
      ...
      forkJoin(client1$, client2$).subscribe(
        ([message1, message2]: [any, any]) => {
    ...
    test('passes through middlewares', async (done) => {
      ...
      const m$: WsMiddlewareEffect = (event$) =>
        event$.pipe(
          tap(
            (event) =>
              event.payload !== undefined && (event.payload as number)++
          )
        );
    ...
    test('passes error (thrown by invalid JSON object) through stream multiple times', async (done) => {
      ...
      const outgoingEvent = {
        ...
        error: {
          name: 'SyntaxError',
          message: 'Unexpected token s in JSON at position 2',
        },
      };
    ...
    test('passes error (thrown by effect) through stream multiple times', async (done) => {
      ...
      const outgoingEvent = {
        type: 'UNHANDLED_ERROR',
        error: { name: 'Error', message: 'test_message' },
      };
