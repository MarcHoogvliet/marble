...
describe('messagingServer::AMQP', () => {
  ...
  test("acks subssequent events and doesn't block the consumer", async (done) => {
    ...
    const optionsClient = Util.createAmqpOptions({
      queue: optionsMicroservice.queue,
    });
    ...
    // when
    microservice = await Util.createAmqpMicroservice(optionsMicroservice)({
      effects: [ack$],
      output$,
    });
  ...
  test("rejects unhandled event events and doesn't block the consumer", async (done) => {
    // given
    const optionsMicroservice = Util.createAmqpOptions({
      expectAck: true,
      timeout: 100,
    });
    const optionsClient = Util.createAmqpOptions({
    ...
    // then
    const output$ = Util.assertOutputEvent({ type: 'ACK_RESPONSE' })(done);
  ...
  test('chains events by sending back to origin channel when no reply is defined', async (done) => {
    ...
    const test1$: MsgEffect = (event$) =>
      event$.pipe(matchEvent('TEST_1'), delay(25), mapTo({ type: 'TEST_2' }));

    const test2$: MsgEffect = (event$) =>
      event$.pipe(matchEvent('TEST_2'), delay(25), mapTo({ type: 'TEST_3' }));
  ...
  test("sends outgoing event to different channel and doesn't cause infinite loop", async (done) => {
    ...
    const test$: MsgEffect = (event$) =>
      event$.pipe(matchEvent('TEST'), map(reply(replyTo)));
