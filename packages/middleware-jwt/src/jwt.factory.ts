...
type VerifyTokenHandler = <T extends Record<string, unknown>>(
  opts: VerifyOptions
) => (token: string) => Observable<T & IssuedAt>;

export const generateExpirationInHours = (hours = 1) =>
  Math.floor(Date.now() / 1000) + 60 * 60 * hours;

export const generateToken = ({ secret, ...opts }: GenerateOptions) => (
  payload: Payload = {}
...
export const verifyToken = <T extends Record<string, unknown>>({
  secret,
  ...opts
}: VerifyOptions) => (token: string) =>
  jwt.verify(token, secret, opts) as T & IssuedAt;

export const verifyToken$: VerifyTokenHandler = <
